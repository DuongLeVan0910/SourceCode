...
const Attendance = () => {
    ...
    const fetchSessions = async () => {
        // Giữ nguyên như cũ
        try {
            const response = await fetch('http://localhost/doanne/backend/get_sessions.php');
    ...
    const fetchStudentsBySession = async () => {
        // Giữ nguyên như cũ
        try {
            const response = await fetch(`http://localhost/doanne/backend/get_students_by_session.php?session_id=${selectedSession}`);
            if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
            if (data.success) {
