    ...
    if (password_verify($password, $user["password"]) && $user["role"] === $role) {
        ...
        if ($user["role"] === "student") {
            ...
            if ($student_result->num_rows > 0) {
                ...
                // Lấy session_id từ bảng class_session
                $mssv = $student_info['mssv'];
                $stmt = $conn->prepare("SELECT session_id FROM class_session_students WHERE mssv = ?");
                ...
                if ($session_result->num_rows > 0) {
                    ...
                    $student_info['session_id'] = $session['session_id'];
                } else {
                    $student_info['session_id'] = null;
                }
            }
        }
