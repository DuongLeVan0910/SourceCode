...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (
        !$data ||
        !isset($data['date']) || !isset($data['time_slot']) || !isset($data['room']) ||
        !isset($data['subject_name']) || !isset($data['start_date']) || !isset($data['end_date'])
    ) {
        echo json_encode(["success" => false, "message" => "Dữ liệu không hợp lệ hoặc thiếu thông tin"]);
        exit();
    }
    ...
    $conn->begin_transaction();
    try {
        $sql = "INSERT INTO class_sessions (date, time_slot, room, subject_name, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssss", $date, $time_slot, $room, $subject_name, $start_date, $end_date);
        $stmt->execute();
        $session_id = $conn->insert_id;
